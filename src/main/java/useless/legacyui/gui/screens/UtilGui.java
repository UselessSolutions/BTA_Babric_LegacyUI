...
public class UtilGui {
    ...
    public static void drawPanorama(final Screen gui){
        ...
        tessellator.startDrawingQuads();
        final float brightness = LegacyUI.modSettings.legacyui$getMainMenuBrightness().value;


        final int scrollLength = (1000 * ((LegacyUI.modSettings.legacyui$getPanoramaScrollLength().value + 1) * 15));
    ...
    public static float getHotbarAlpha(){
        ...
        if (!LegacyUI.modSettings.legacyui$getEnableHUDFadeout().value) return 1;
        if (mc.thePlayer.inventory.getCurrentItemIndex() != prevItem) {
            timeHotbarLastActive = System.currentTimeMillis();
        }
        prevItem = mc.thePlayer.inventory.getCurrentItemIndex();
        final long currentTime = System.currentTimeMillis();
        if (currentTime - timeHotbarLastActive <= (LegacyUI.modSettings.legacyui$getHUDFadeoutDelay().value * 10000)){
            return 1f;
        }
        return Math.max(1f - ((currentTime - timeHotbarLastActive) - (LegacyUI.modSettings.legacyui$getHUDFadeoutDelay().value * 10000))/1000f, LegacyUI.modSettings.legacyui$getHUDFadeoutAlpha().value);
    }
}
