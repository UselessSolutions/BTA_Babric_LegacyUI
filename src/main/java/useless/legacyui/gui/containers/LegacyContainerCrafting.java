...
//        Random random = new Random();
        ...
        for (final RecipeGroup group : craftingGroups){
            ...
            if (index == currentSlotId){ // special rendering for scrolling and recipe preview
                ...
                this.addSlot(new SlotCraftingDisplayLegacy(this.slots.size(), 12 + 18 * index, 56, item, discovered, !craftable, LegacyUI.modSettings.legacyui$getHighlightColor().value.value));
                ...
                if (group.getRecipes(isInInventory).size() > 1) { // If multiple Items in recipe group
                    ...
                    this.addSlot(new SlotCraftingDisplayLegacy(this.slots.size(), 12 + 18 * index, 56 + 21, item, discovered, !craftable, LegacyUI.modSettings.legacyui$getHighlightColor().value.value));
                ...
                item = (ItemStack) recipe.getOutput();
                discovered = isDicovered(item, statCounter, player);
                this.addSlot(new SlotCraftingDisplayLegacy(this.slots.size(), 103, 123+offset, item, discovered, !craftable, LegacyUI.modSettings.legacyui$getHighlightColor().value.value, 26));
                ...
                for (int j = 0; j < inputSymbols.length; j++) {
                    ...
                    if (inputSymbols.length > 5){
                        ...
                        this.addSlot(new SlotCraftingDisplayLegacy(this.slots.size(), 20 + 18 * (j % 3), 109 + offset + 18 * (j / 3), item, discovered, InventoryHelper.itemsInInventory(this.inventoryPlayer, itemSymbol) <= num && item != null, LegacyUI.modSettings.legacyui$getHighlightColor().value.value));
                    }
                    else {
                        // Render 2x2 crafting gird
                        this.addSlot(new SlotCraftingDisplayLegacy(this.slots.size(), 29 + 18 * (j % 2), 118 + offset + 18 * (j / 2), item, discovered, InventoryHelper.itemsInInventory(this.inventoryPlayer, itemSymbol) <= num && item != null, LegacyUI.modSettings.legacyui$getHighlightColor().value.value));
                    }
    ...
    public static boolean isDicovered(final ItemStack item, final StatsCounter statWriter, final Player player){
        if (!LegacyUI.modSettings.legacyui$getCraftingHideUndiscoveredItems().value){
            return true;
        }
